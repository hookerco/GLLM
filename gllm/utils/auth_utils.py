...
def resolve_openrouter_api_key(
    *,
    ...
    secrets = _default_secrets() if secrets is None else secrets
    return _first_nonempty(
        _mapping_get(env, "OPENROUTER_API_KEY"),
        _mapping_get(env, "OPENROUTER_TOKEN"),
        _mapping_get(secrets, "openrouter_token"),
    )


def resolve_streamlit_secret(
    key: str,
    *,
    secrets: Optional[Mapping[str, str]] = None,
) -> Optional[str]:
    secrets = _default_secrets() if secrets is None else secrets
    return _mapping_get(secrets, key)
